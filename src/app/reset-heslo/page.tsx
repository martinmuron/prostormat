  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full">
        ...
        <p className="text-body text-gray-600 mb-6">Zadejte nové heslo pro váš účet.</p>
        {success ? (
          <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-green-800">
            Heslo bylo úspěšně změněno. <Link className="underline" href="/prihlaseni">Přihlásit se</Link>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={isLoading} className="w-full">{isLoading ? "Ukládám..." : "Změnit heslo"}</Button>
          </form>
        )}
      </div>
    </div>
