...
export function AdminVenueEditForm({ venue }: AdminVenueEditFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First, try to find and update manager by email if it changed
      if (managerEmail !== venue.manager?.email) {
        const userResponse = await fetch(`/api/admin/users/find-by-email?email=${encodeURIComponent(managerEmail)}`)

        if (userResponse.ok) {
          const userData = await userResponse.json()
          formData.managerId = userData.user.id
        } else {
          setErrorMessage(`Uživatel s emailem ${managerEmail} nebyl nalezen`)
          setIsLoading(false)
          return
        }
      }
      ...
      if (response.ok) {
        setSuccessMessage("Prostor byl úspěšně aktualizován!")
        setTimeout(() => {
          router.refresh()
        }, 1500)
      } else {
