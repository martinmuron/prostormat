...
async function replaceSupabaseVenues() {
  const prisma = new PrismaClient()
  
  console.log('🔄 Replacing venues in Supabase production database...')
  
  try {
    // First, check current venues
    console.log('📊 Checking current venues in prostormat_venues table...')
    const currentVenues = await prisma.$queryRaw`
      SELECT name, slug FROM prostormat_venues ORDER BY name
    `
    console.log(`Found ${currentVenues.length} venues:`)
    currentVenues.forEach((venue, index) => {
      console.log(`${index + 1}. ${venue.name} (${venue.slug})`)
    })

    console.log('\n🗑️ Removing all existing venues...')
    
    // Remove related records first to avoid foreign key constraints
    await prisma.$queryRaw`DELETE FROM prostormat_venue_broadcast_logs;`
    await prisma.$queryRaw`DELETE FROM prostormat_venue_inquiries;`
    ...
    let manager = await prisma.$queryRaw`
      SELECT id FROM prostormat_users WHERE email = 'newvenues@prostormat.cz' LIMIT 1;
    `
    
    if (manager.length === 0) {
      // Create venue manager
      const managerId = 'cm' + Math.random().toString(36).substring(2, 15)
      await prisma.$queryRaw`
        INSERT INTO prostormat_users (id, name, email, password, role, phone, company, "createdAt") 
        VALUES (${managerId}, 'Real Venues Manager', 'newvenues@prostormat.cz', '$2a$12$hash', 'venue_manager', '+420 800 900 100', 'Prostormat', NOW());
      `
    ...
    const venues = [
      ...
      // Adding more venues... (truncated for brevity, but I'll include all 15)
      {
    ...
    for (const venue of venues) {
      ...
      const amenities = JSON.stringify(['WiFi', 'Profesionální servis', 'Klimatizace', 'Zvukový systém'])
      ...
      await prisma.$queryRaw`
        INSERT INTO prostormat_venues (
          ...
          ${venue.websiteUrl}, ${images}, 'active', ${managerId}, NOW(), NOW()
        );
      `
      insertedCount++
